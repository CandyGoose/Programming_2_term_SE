...
/**
 * Удаление первой персоны из коллекции
 */
        ...
        try {
            if (dbManager.validateUser(request.getLogin(), request.getPassword())) {
                if (collectionManager.getCollection().isEmpty())
                    ...
                    Integer firstPersonId = collectionManager.getCollection().getFirst().getId();
                    if (dbManager.checkPersonExistence(firstPersonId)) {
